...
//        Document studentInfoView = getStudentInfoView();


        ArrayList<ArrayList<ArrayList<String>>> obligatoryCourse = getObligatoryCourse(studentInfoView);

        ArrayList<ArrayList<String>> electiveCourse = getElectiveCourse(studentInfoView);

        ArrayList<String> creditSituation = getCreditSituation(studentInfoView);

        ArrayList<ArrayList<courseVo>> obligatoryCourseList = obligatoryCourse.stream()
        ...
        ArrayList<courseVo> electiveCourseList = electiveCourse.stream()
                .map(courseData -> {
                    courseVo course = new courseVo();
        ...
        creditSituationVo creditSituationVo = new creditSituationVo(creditSituation.get(0), creditSituation.get(1), creditSituation.get(2), creditSituation.get(3), creditSituation.get(4), creditSituation.get(5));
        // 整合并返回数据
        Map<String, Object> data = new HashMap<>();
...
//        String url = new String(ATTENDANCE_URL + "&studentID=" + dynamicCode.get("studentID") + "&gzcode=" + dynamicCode.get("gzcode"));
        //获取document
    ...
    public List<RegularGradeVo> getRegularGrade(StudentDTO student) throws IOException {
        ...
        //处理数据存入对象
        doc.select(".tableBodyleft").parallelStream().forEach(element -> {
...
//                .parse();
    ...
    /**
     * 获取动态代码StudentId和gzCode
     * 代替了getStudentInfoView方法，获取参数更多，
     *
     * @return
     */
    ...
    private ArrayList<ArrayList<String>> getElectiveCourse(Document doc) {
        ...
        for (Element tr : trs) {
            Elements tdList = tr.select("tr > td:not(:nth-child(5), :nth-child(7), :nth-child(8), :nth-child(9))");
            ArrayList<String> strings = new ArrayList<>();
            for (Element td : tdList) {
                String text = td.text();
                strings.add(text);
    ...
    private ArrayList<String> getCreditSituation(Document doc) {
        ...
        Elements trs = tbody.select("tbody > tr:not(:nth-child(1), :nth-child(2), :nth-child(9), :nth-child(10))");
